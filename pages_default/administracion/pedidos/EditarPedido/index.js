...
import { ArrowBack, Assignment, AttachMoney, Close, ErrorRounded, Visibility } from "@material-ui/icons";
import { Box, Button, FormControlLabel, Grid, IconButton, LinearProgress, Paper, Switch } from "@material-ui/core";
...
import { quotesSelector, setArrayExtraProducts } from "../../../../redux/slices/quotesSlice";
...
import { Error, SeeOrderContainer, TableProducts } from "../../../../styles/Administracion/pedidos/editarPedido";
...
export default function EditarPedido() {
  ...
  const { isLoadingPage } = useValidateLogin(["administracion"]);
  ...
  const getDataOrder = async () => {
    try {
      ...
      let params = {
        include:
          "createdbyid,oportunity,oportunity.prospect,oportunity.productsoportunities,address,address.postal,address.city,address.entity,bill,bill.cfdi,bill.paymentmethod,bill.paymentway,bill.address,bill.taxregime,paymentaccount",
      };
    ...
    try {
      for (let i = 0; i < add.length; i++) {
        ...
        let responseFiles = await api.post(
          `files/uploadtofolder/${dataOrder?.oportunity?.prospectId}/${fileToSave.name}${fileToSave.type}`,
          newData
        );
  ...
  const saveChances = async (item, formData) => {
    ...
    try {
      ...
      if (dataOrder.orderstatusId == "CwNWIj2RxW6N2B9v4WiwD1V9") {
                  ...
                  <Grid item xs={12}>
                    <div className="primary">
                      ...
                      <FormControlLabel
                        control={
                          <Switch
                            disabled={dataOrder?.billing ? true : false}
