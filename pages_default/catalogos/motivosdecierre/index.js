...
import ClossingTable from "../../../components/TableClossingReasons";
...
export default function Origins(page, isLoading, handleAlert, setAlert) {
  ...
  const [totalClossing, setTotalClosing] = useState(0);
  const [showFilters, setshowFilters] = useState(false);
  const [searchKey, setSearchKey] = useState("");
  const [filterClossing, setFilterClossing] = useState("");
  ...
  const navigateCreateNew = () => {
    router.push("../catalogos/motivosdecierre/nuevo");
  };
        ...
        <div>
          {filterClossing !== "" && (
            <Chip
              ...
              label={`${"Buscando por Motivo de cierre"}`}
              className="chip"
            />
          )}
          {searchKey && (
  ...
  return (
    <DirectorLayout>
      <OriginStyled>
        <Head>
          <title>CRM JOBS -Motivos de Cierre</title>
        </Head>
        {/* <SideBar />
          ...
          <div className="container">
            <div className="head">
              <div className="head__title">
                <div className="total">
                  <h1>
                    <ClosedCaptionOutlined className="icon" /> Motivos de cierre
                  </h1>
                  <a>({`${totalClossing} registros`})</a>
            ...
            <ClossingTable
              totalClossing={totalClossing}
              setTotalClosing={setTotalClosing}
              searchKey={searchKey}
              filterClossing={filterClossing}
              flag={flag}
          ...
          <div className="ctr_drawer">
            ...
            <Grid container className="container" spacing={2}>
              <Grid item md={12} className="casilla">
                ...
                <label className="label2"> Buscar por Motivo de cierre </label>

                <input
                  onChange={e => {
                    setFilterClossing(e.target.value);
                  }}
                  type="checkbox"
                  className="checkBox"
                />
