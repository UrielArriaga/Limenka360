...
export default function NewOrigin() {
  ...
  const createData = async formData => {
    try {
      setAlert({ severity: "info", show: true, message: "Un momento - Creando canal", type: "load" });
      let body = {};
      body.name = formData.name.toLocaleLowerCase();
      body.companyId = "62dz3qnimTqzfPfKpt7JtOtE";
      console.log(body);
      let newChanel = await api.post("channels", body);
      if (newChanel.status === 201) {
        setAlert({ severity: null, show: false, message: null, type: null });
        handleAlert("success", "Canal - ¡Agregado correctamente!", "basic");
      ...
      switch (error.request?.status) {
        case 401:
          return handleAlert("erroror", "Canales - ¡No cuentas con las credenciales!", "basic");
        case 403:
          return handleAlert("error", "Canales - ¡No tienes permisos!", "basic");
        case 404:
          return handleAlert("error", "Canales - ¡Ruta no encontrada!", "basic");

        default:
          return handleAlert("error", "Canales - ¡Error al cargar los datos!", "basic");
      }
    }
  };
  ...
  return (
    <DirectorLayout>
      <NewOriginStyled>
        <Head>
          <title>CRM JOBS - Nuevo Canales</title>
        </Head>
        {/* <NavBarDashboard /> */}
          ...
          <div className="main_prospects">
            <div className="head">
              <Tooltip title="Regresar">
                <ArrowBack className="arrow" onClick={() => router.push({ pathname: `/catalogos/canales` })} />
              </Tooltip>
              <h1> Agregar Canales</h1>
            </div>{" "}
