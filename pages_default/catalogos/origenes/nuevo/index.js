...
export default function NewOrigin() {
  ...
  const navigateBack = () => {
    router.push("../origenes");
  };

  const createData = async (formData, e) => {
    try {
      setAlert({ severity: "info", show: true, message: "Un momento - Creando origen", type: "load" });

      let body = {};
      body.name = formData.name.toLocaleLowerCase();
      body.companyId = company;
      console.log(body);
      let newOrigin = await api.post("origins", body);
      if (newOrigin.status == 201) {
        handleAlert("success", "Origen - ¡Agregado correctamente!", "basic");
        console.log(newOrigin.data);
        setTimeout(() => {
          router.push("../origenes");
        }, 2000);
      }
      ...
      switch (err.request?.status) {
        case 401:
          return handleAlert("error", "Origen - ¡No cuentas con las credenciales!", "basic");
        case 403:
          return handleAlert("error", "Origen - ¡No tienes permisos!", "basic");
        case 404:
          return handleAlert("error", "Origen - ¡Ruta no encontrada!", "basic");

        default:
          return handleAlert("error", "Origen - ¡Error al cargar los datos!", "basic");
      }
    }
  };
  ...
  return (
    <DirectorLayout>
      <NewOriginStyled>
        <Head>
          <title>CRM JOBS - Nuevo Origen</title>
        </Head>
        {/* <NavBarDashboard /> */}
          ...
          <div className="main_prospects">
            <div className="head">
              <Tooltip title="Regresar">
                <ArrowBack className="arrow" onClick={() => router.push({ pathname: `/catalogos/origenes` })} />
              </Tooltip>
