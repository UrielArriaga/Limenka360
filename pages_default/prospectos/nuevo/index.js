...
export default function NewProspect() {
  ...
  const {
    ...
    channels,
    users,
    clientsCompanies: datacompanies,
  ...
  const [valuecompanies, setValueCompanies] = useState("");
  ...
  const getClientscompanies = async () => {
    try {
      ...
      // setDatacompanies(clientsCompany.data.results);
  ...
  const handleCreateProspect = async (formData, returnTo) => {
    try {
      ...
      formData.bidder = formData.bidder;
      // se agrego company 27/10/2022

      formData.clientCompanyId = companySeleccionado.id;
      ///-----------------//////
      ...
      if (prospectNew.status == 201) {
        ...
        //     type: "create",
        //     from: "prospects",
        //     message: `Nuevo prospecto ${sendType} creada desde ${sendFrom}`,
        //     data: { prospectNew },
        //     ejecutiveId: id_user,
        //     groupId: groupId,
        //     companyId: company,
  ...
  const handleOpenAddCompanies = event => setOpenAdd(!openAdd);

  const onSuggestionsFetchRequested = ({ value }) => setClientsCompanies(filtrarcompanies(value));

  const filtrarcompanies = value => {
    const inputValue = value.trim().toLowerCase();
    const inputLength = inputValue.length;

    var filtrado = datacompanies.results.filter(company => {
      var textoCompleto = company.companyname;
  ...
  const onSuggestionsClearRequested = () => setClientsCompanies([]);
  ...
  const renderSuggestion = suggestion => (
    <div className="sugerencia" onClick={() => seleccionarcompany(suggestion)}>
      {toUpperCaseChart(`${suggestion.companyname}`)}
    </div>
  );

  const seleccionarcompany = company => setCompanySeleccionado(company);

  const onChange = (e, { newValue }) => setValueCompanies(newValue);

  const inputProps = {
    placeholder: "Busca una Empresa o agrega una nueva",
    className: "input",
  ...
  const eventEnter = e => {
    if (e.key == "Enter") {
      var companyActual = datacompanies.results.filter(p => p.id == e.target.value.trim());
      var company = {
        id: companyActual?.id,
        companyname: companyActual[0]?.companyname ? companyActual[0]?.companyname : "",
        rfc: companyActual[0]?.rfc,
      };
      seleccionarcompany(company);
    }
  };
              ...
              <Grid container className="form">
                ...
                <Grid container>
                  ...
                  <Grid item xs={12} sm={6} md={4}>
                    ...
                    <Checkbox {...register("bidder")} checked={bidder} onChange={e => setBidder(e.target.checked)} />
                  </Grid>
                  <Grid className="refresh">
                ...
                <Grid item xs={12} md={12} className="buttons">
                  ...
                  <Button
                    disabled={isLoaderValidating}
                    onClick={handleSubmit(validateEmail("saveAndNew"))}
                    variant="contained"
                    color="primary"
                    className="btnGuardar"
                    type="submit"
                  ...
                  {companySeleccionado.id === undefined ? (
                    <Button onClick={handleOpenAddCompanies} variant="contained" color="primary" className="btnGuardar">
