...
import ProspectFiles from "../../components/TableProspectFiles";
...
import ModalRestoreProspect from "../../components/ModalRestoreProspect";
...
const Prospecto = () => {
  ...
  const { prospecto, scrollTo, cve } = router.query;
  useEffect(() => {
    let mounted = true;
    if (mounted) {
      getDataInitial();
  ...
  const getDataInitial = async () => {
    try {
      ...
      let Prospect = await api.get(
        `prospects/${prospecto}?include=category,city,entity,phase,origin,clientcompany,clienttype,specialty,postal,ejecutive,ejecutive.group,prospectslabels,prospectslabels.label,channel`
      );
          ...
          <div className="ctr_prospecto">
            ...
            {!isLoading && (
              <div className="ctr_prospecto__info">
                ...
                <div id="archivos">
                  <ProspectFiles
                    scrollTo={router.query.scrollTo === "archivos"}
                    footer={true}
                    prospect={prospect}
                    handleAlert={handleAlert}
                    isProspecto={true}
                ...
                <ModalDiscardProspect
                  isShow={prospect.discarted}
                  name={`${prospect?.name} ${prospect?.lastname}`}
                  email={prospect?.email}
                  id={prospect?.id}
                  handleAlert={handleAlert}
                />
                <ModalRestoreProspect prospect={prospect} isShow={prospect.discarted} refetch={() => setFlag(!flag)} />
              </div>
            )}
            {isLoading && (
