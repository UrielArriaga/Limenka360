...
export default function Login() {
  ...
  const { isFetching, isSuccess, isError, roleId, email, isLogged_User } = useSelector(userSelector);
  ...
  const handleLogin = async data => {
    try {
      ...
      const { _id, name, lastname, role, group, groupId, phone, type, username } = responseLogin.data.data;
      const { tokenSession, refreshToken } = responseLogin.data;

      dispatch(login(responseLogin.data));
      api.defaults.headers.common["Authorization"] = responseLogin.data.tokenSession;
      ...
      <div className="login_container">
        <div className="login_container__form_side">
          ...
          <div className="login_container__form_side__content">
            <form onSubmit={handleSubmit(handleLogin)} className="login_container__form_side__content__container">
              <p className="title">Inciar sesión</p>
              <p className="subtitle">
                Bienvenido a <span>Limenka360</span>, Gestiona las relaciones con tus clientes
              </p>
              <div className="input_item">
                ...
                <div className="input_item__input">
                  ...
                  />
                  {errors?.email && <span className="input_item__input__error">{errors?.email?.message}</span>}
                </div>
              </div>
              <div className="input_item">
                <div className="input_item__label ">
                ...
                <div className="input_item__input">
                  ...
                  />
                  {errors?.password && <span className="input_item__input__error">{errors?.password?.message}</span>}
                </div>
              </div>
              <div className="forgot_password">
                <p onClick={() => handleOpenAlert("Forgot Password", "Contact support to change your password")}>
              ...
              <div className="donthaveaccount">
                <p>
                  ¿No tienes cuenta?{" "}
                  <span onClick={() => handleOpenAlert("Don't have an account?", "Contact your manager")}>
