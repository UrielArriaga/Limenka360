...
const TableComponent = props => {
  ...
  const saveAndRetrivelocalStorage = () => {
    try {
      ...
      let savedHeads = localStorage.getItem("tableShippings");
      if (savedHeads === null || savedHeads === undefined) {
        setHeadsTable(normalizeHeads);
        localStorage.setItem("tableShippings", JSON.stringify(normalizeHeads));
      } else {
                        ...
                        <div className="options">
                          ...
                          {handleClickConfirm && (
                            <div
                              className="options__option"
                              onClick={() => {
                                handleClickConfirm(rowSelected);
                                handleCloseActionsRow();
                              }}
                            >
                              <Check />
...
const TableHeadColumn = styled.th`
  ...
  text-transform: capitalize;
`;
const TableHeadColumnProduct = styled.th`
  min-width: 340px;
  text-align: left;
  padding-left: 10px;
  text-transform: capitalize;
...
const ModalSettings = ({ open, setOpen, headsTable, setHeadsTable, id, heads }) => {
  ...
  const saveUpdatesTable = data => {
    localStorage.setItem("tableShippings", JSON.stringify(data));
  };
