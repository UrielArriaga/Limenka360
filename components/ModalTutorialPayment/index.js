...
export default function ModalTutorialPayment() {
  ...
  const ShowModalLimenka = () => {
    let modalimenka = localStorage.getItem("modal-payments");

    if (modalimenka === null || modalimenka === undefined) {
      setOpenModalLimenka(true);
      localStorage.setItem("modal-payments", JSON.stringify(true));
      console.log(modalimenka);
    } else {
      localStorage.setItem("modal-payments", JSON.stringify(false));
      setOpenModalLimenka(false);
    }
  };
      ...
      <Dialog open={openModalLimenka}>
        <ModalLimenka>
          ...
          <Grid className="Container" container>
            <Grid item md={12}>
              <img style={{ width: "100%" }} src="/tutorials/payments.gif" alt="" />
            </Grid>
          </Grid>
