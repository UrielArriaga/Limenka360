...
const ChannelsTable = ({ totalChanel, setTotalChanel, searchKey, filterChanel, flag }) => {
  ...
  const getData = async () => {
    try {
      ...
      let canal = await api.get(`channels?${queryString}&${count}&${limite}&${skip}&${order}`);
      setTotalChanel(canal.data.count);
      normalizeData(canal.data.results);
      console.log(canal.data.results);
    } catch (error) {
      setIsLoading(false);
  ...
  const uploadData = canal => {
    setDataToUpdate(canal);
    setshowUpload(true);
    setValue("name", canal.name);
    setName(canal.name);
  };
  ...
  const handleUpdateData = formData => {
    ...
    api.put(`channels/${dataToUpdate.id}`, jsonUpdate).then(res => {
      console.log(res);
      setRefetch(!refetch);
      resetForm();
      handleClose();
  ...
  const deleteData = canal => {
    setDataToDelete(canal);
    setShowDelete(true);
  };

  const handleDeleteData = () => {
    api.delete(`channels/${dataToDelete.id}`).then(res => {
      setRefetch(!refetch);
      handleCloseDelete();
    });
  };
  ...
  const handleDeleteMany = () => {
    dataToManyDelete.forEach(item => {
      api.delete(`channels/${item.id}`).then(res => {
        setShowDeleteMany(false);
  ...
  return (
    <TableChanels>
      ...
      >
        <DialogContainer>
          <p className="title">Editar Canal</p>
          {Inputs()}
      ...
      >
        <DialogContainer>
          <p className="title">Eliminar Canal {dataToDelete?.name}</p>
      ...
      >
        <DialogContainer>
          <p className="title">Eliminar Canales</p>
          <Grid container className="ctr_buttons" style={{ justifyContent: "center" }}>
