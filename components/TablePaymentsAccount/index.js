...
const PaymentAccountTable = ({ totalPhases, setTotalPhases, searchKey, flag, filterPhase }) => {
  ...
  const getData = async () => {
    try {
      ...
      console.log(limite);
      let phase = await api.get(`paymentsacounts?${queryString}&${count}&${limite}&${skip}&${order}`);
      setTotalPhases(phase.data.count);
      normalizeData(phase.data.results);
    } catch (error) {
      setIsLoading(false);
      handleAlert("error", "Cuentas por cobrar - Error al cargar los datos!", "basic");
    }
  };
  ...
  const handleDeletePhase = () => {
    api.delete(`paymentsacounts/${dataToDelete.id}`).then(res => {
      setRefetch(!refetch);
      handleCloseDelete();
    });
  };
  ...
  const handleUploadPhase = formData => {
    ...
    api.put(`paymentsacounts/${dataToUpdate.id}`, jsonToUpdate).then(res => {
      setRefetch(!refetch);
      resetForm();
      handleClose();
    });
  ...
  return (
    <TablePhase>
      {isLoading ? (
        ...
        <>
          <TableCustom
            ...
            keyJson="paymentsAccount"
            primaryColor={"#405189"}
            secondaryColor={"#dce1f6"}
            keyStorage="paymentsAccount"
            actionsPerItem={[
      ...
      >
        <DialogContainer>
          <p className="title">Editar Cuenta por cobrar</p>
          {Inputs()}
