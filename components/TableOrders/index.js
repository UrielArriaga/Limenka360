...
import { Badge, Button, Chip, Dialog, Divider, Menu, Popover, Tooltip } from "@material-ui/core";
import {
  AddAlert,
  AttachMoney,
  Check,
          ...
          <div className="content ">
            <div className="content__flex">
              <div className="title_name">
                ...
                {itemData?.data?.isverified && <span className="verified">Verificado</span>}
              </div>
      ...
      return (
        <TableDataId isPar={isPar} isNew={isNew} className="column_id">
          <div className="content">
            ...
            <div className="content__more">
              <p className="txt-lastracking">
                Ultima vez <span>{getDiferrenceDates(itemData["fecha de creacion"])} </span>
              </p>
            </div>
          </div>
        </TableDataId>
...
const TableComponent = props => {
  ...
  const {
    ...
    approveOrder,
    rejectOrder,
    handleClickApproveDisapprove,
    handleClickVerify,
    handleClickRemoveVerify,
  } = props;
  ...
  const saveAndRetrivelocalStorage = () => {
    try {
      ...
      let savedHeads = localStorage.getItem("tableOrders");
      if (savedHeads === null || savedHeads === undefined) {
        setHeadsTable(normalizeHeads);
        localStorage.setItem("tableOrders", JSON.stringify(normalizeHeads));
      } else {
                    ...
                    {discartedTable ? (
                      ...
                      >
                        <div className="options">
                          ...
                          {handleClickVerify && rowSelected?.data?.isverified === false && (
                            <div
                              className="options__option"
                              onClick={() => {
                                handleClickVerify(rowSelected);
                                handleCloseActionsRow();
                              }}
                            >
                              <ThumbUpIcon />
                          ...
                          {handleClickRemoveVerify && rowSelected?.data?.isverified === true && (
                            <div
                              className="options__option"
                              onClick={() => {
                                handleClickRemoveVerify(rowSelected);
                                handleCloseActionsRow();
                              }}
                            >
                              <Close />
                              <p>Remover de Verificados</p>
                            </div>
                          )}
                          {approveOrder && (
                            <div
...
const ModalSettings = ({ open, setOpen, headsTable, setHeadsTable, id, heads }) => {
  ...
  const saveUpdatesTable = data => {
    localStorage.setItem("tableOrders", JSON.stringify(data));
  };
