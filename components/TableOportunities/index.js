...
const TableProspects = ({
  ...
  const saveInLocalStorage = (headersCustom) => {
    ...
    if (headsSaveInStorage && storage == headersLocal) {
      setheadCell(JSON.parse(headsSaveInStorage));
      setIsLoadingTable(false);
    } else {
                      ...
                      <th className="title checkbox" key={index}>
                        {selectmultiple ? (
                          itemsSelect.length > 0 && itemsSelect.length < data.length ? (
                            <FormControlLabel
                              control={<Checkbox checked={data.length !== itemsSelect.length ? true : false} indeterminate size="small" onChange={(e) => setitemsSelect([])} />}
                            />
                          ) : (
                            <FormControlLabel
                              control={<Checkbox checked={data.length == itemsSelect.length ? true : false} size="small" onChange={(e) => checkedItemAll(e.target.checked)} />}
                            />
                          )
                        ) : null}
