...
export default function ModalDeletePotential({ open, close, toggle, setFlag, flag, oportunity }) {
  ...
  const handleAddPottential = async () => {
    ...
    try {
      let responserejectedRestore = await api.put(`prospects/${oportunity?.prospectId}`, { isclientpotencial: false });
      if (responserejectedRestore.status === 200) {
        handleGlobalAlert(
          "success",
          "El prospecto elimino de cliente potencial correctamente",
          "basic",
          dispatch,
          6000
        );
      ...
      handleGlobalAlert(
        "error",
        "opotunidad -ocurrio un error al eliminar de cliente potencial",
        "basic",
        dispatch,
        6000
      );
    ...
    >
      <DialogContainer>
        ...
        <Grid spacing={1} container className="ctr_inputs">
          <p className="title">El Prospecto se eliminara de cliente potencial</p>
        </Grid>
