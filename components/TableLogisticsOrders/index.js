...
const TableDataComponent = ({
  ...
  switch (headName) {
    ...
    case "estado":
      return (
        <TableDataId isPar={isPar} isNew={isNew} className="column_id">
          <div className="content ">{applyStatusStyle(item)}</div>
        </TableDataId>
      );
...
const TableComponent = props => {
  ...
  const saveAndRetrivelocalStorage = () => {
    try {
      ...
      let savedHeads = localStorage.getItem("tableOrders-logistics");
      if (savedHeads === null || savedHeads === undefined) {
        setHeadsTable(normalizeHeads);
        localStorage.setItem("tableOrders-logistics", JSON.stringify(normalizeHeads));
      } else {
...
const ModalSettings = ({ open, setOpen, headsTable, setHeadsTable, id, heads }) => {
  ...
  const saveUpdatesTable = data => {
    localStorage.setItem("tableOrders-logistics", JSON.stringify(data));
  };
