...
const ReasonsTable = ({ totalReasons, setTotalReasons, searchKey, filterReason, flag }) => {
  ...
  const totalPages = Math.ceil(totalReasons / limit);
  ...
  const getData = async () => {
    try {
      ...
      let reason = await api.get(`reasons?${queryString}&${count}&${limite}&${skip}&${order}`);
      setTotalReasons(reason.data.count);
      normalizeData(reason.data.results);
    } catch (error) {
      setIsLoading(false);
      handleAlert("error", "Razones - Error al cargar los datos!", "basic");
    }
  };
  ...
  const normalizeData = data => {
    ...
    for (let i = 0; i < data?.length; i++) {
      ...
      itemNormalize.name = formatUpperCase(element?.reason);
  ...
  const uploadData = motivo => {
    setDataToUpdate(motivo);
    setshowUpload(true);
    setValue("name", motivo.name);
    setName(motivo.name);
  };

  const handleUpdateData = formData => {
    let jsonUpdate = {};
    jsonUpdate.reason = formData.name;
    api.put(`reasons/${dataToUpdate.id}`, jsonUpdate).then(res => {
      console.log(res);
      setRefetch(!refetch);
      resetForm();
      handleClose();
  ...
  const deleteData = motivos => {
    setDataToDelete(motivos);
    setShowDelete(true);
  };

  const handleDeleteData = () => {
    api.delete(`reasons/${dataToDelete.id}`).then(res => {
      setRefetch(!refetch);
      handleCloseDelete();
    });
  };
  ...
  const handleDeleteMany = () => {
    dataToManyDelete.forEach(item => {
      api.delete(`reasons/${item.id}`).then(res => {
        setShowDeleteMany(false);
  ...
  return (
    <TableReasons>
      ...
      >
        <DialogContainer>
          <p className="title">Editar Motivo</p>
          {Inputs()}
      ...
      >
        <DialogContainer>
          <p className="title">Eliminar Motivo {dataToDelete?.name}</p>
      ...
      >
        <DialogContainer>
          <p className="title">Eliminar Motivos</p>
          <Grid container className="ctr_buttons" style={{ justifyContent: "center" }}>
