            ...
            <tbody className="ctr_table__body">
              {data.map((item, index) => {
                ...
                return (
                  <tr
                    key={index}
                    className={item.viewed ? (checkrow(index) ? "row" : "row inpar") : `new_row ${checkrow(index) ? "row" : "row inpar"}`}
                  >
                    {headCell.map((i, ix) => {
                        ...
                        return (
                          <td className="data" key={ix}>
                            {typeof cell[ix] !== "object" && (
                              <p className="ctr_td">
                                {keyCell[ix]?.includes("correo") ||
                                  (keyCell[ix]?.includes("email") && <MailOutline fontSize="small" className="icon_mail" />)}
                                {keyCell[ix]?.includes("phone") ||
                                  (keyCell[ix]?.includes("movil") && <Smartphone fontSize="small" className="icon_phone" />)}
            ...
            {headersActives.map((item, index) => {
              if (item.text !== "id") {
                return (
                  <div
                    key={index}
                    className={checkrow(index) ? "ctr_dialog__list_headers__ctr_head" : "ctr_dialog__list_headers__ctr_head inpar"}
