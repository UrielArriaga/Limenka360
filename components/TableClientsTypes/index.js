...
const TypeClientsTable = ({ totalChanel, setTotalChanel, searchKey, filterChanel, flag }) => {
  ...
  const totalPages = Math.ceil(totalChanel / limit);
  ...
  const getData = async () => {
    try {
      ...
      let client = await api.get(`clienttypes?${queryString}&${count}&${limite}&${skip}&${order}`);
      setTotalChanel(client.data.count);
      normalizeData(client.data.results);
      console.log(client.data.results);
    } catch (error) {
      setIsLoading(false);
      handleAlert("error", "Tipos de clientes - Error al cargar los datos!", "basic");
    }
  };
  ...
  const uploadData = client => {
    setDataToUpdate(client);
    setshowUpload(true);
  ...
  const handleUpdateData = formData => {
    ...
    api.put(`clienttypes/${dataToUpdate.id}`, jsonUpdate).then(res => {
      console.log(res);
      setRefetch(!refetch);
      resetForm();
      handleClose();
  ...
  const handleDeleteData = () => {
    api.delete(`clienttypes/${dataToDelete.id}`).then(res => {
      setRefetch(!refetch);
      handleCloseDelete();
    });
  };
  ...
  const handleDeleteMany = () => {
    dataToManyDelete.forEach(item => {
      api.delete(`clienttypes/${item.id}`).then(res => {
        setShowDeleteMany(false);
  ...
  return (
    <TableTypeClients>
      {isLoading ? (
        ...
        <>
          <TableCustom
            ...
            keyJson="tipos_de_clientes"
            keyStorage="tipos_de_clientes"
            actionsPerItem={[
      ...
      >
        <DialogContainer>
          <p className="title">Editar Tipo de Cliente</p>
          {Inputs()}
      ...
      >
        <DialogContainer>
          <p className="title">Eliminar Tipo de Cliente </p>
...
export default TypeClientsTable;

const TableTypeClients = styled.div`
  .ctr_load {
    display: flex;
