...
export default function SalesClientsPage({ sales }) {
  const router = useRouter();
  const show_by_default = 20;
  const [showAll, setShowAll] = useState(false);
  const visibleOptions = showAll ? sales.length : show_by_default;
  const toggleShowAll = () => {
    setShowAll(!showAll);
  };
  ...
  const handleClickProspect = item => {
    router.push({
      ...
      query: { prospecto: item.prospectId },
    });
  };
  return (
    <SalesPageStyled>
      {sales.length === 0 && (
        <div className="emptymessage">
          <p>No hay ventas por el momento</p>
        </div>
      )}
      {sales.slice(0, visibleOptions).map((item, index) => (
        <div key={index} className="salesCard">
          <div className="top">
            <div className="item">
      ...
      {sales.length > show_by_default && (
        <div className="containerButtons">
          <button className="viewMore" onClick={toggleShowAll}>
            {!showAll ? `+${sales.length - show_by_default} Ver Más` : "Ver Menos"}
          </button>
        </div>
      )}
    </SalesPageStyled>
