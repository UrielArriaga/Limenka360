...
import { Cached, Cancel, Close, FilterList, People, Info, SearchOutlined } from "@material-ui/icons";
import {
  Box,
  Button,
  Chip,
...
import { AlertDate, FiltersSales, SalesStyled, AlertDeleteStyle } from "./style";
...
export default function ExecutiveSales() {
  ...
  const { phases, origins, clientTypes, categories, clientsCompanies, labels, entities } = useSelector(commonSelector);
  ...
  const { id_user, roleId, userData } = useSelector(userSelector);
  ...
  const getLocalStorage = () => {
    let filtersExecutive = localStorage.getItem("s_FiltersExecutive");
    if (filtersExecutive) {
    ...
    let orderby = localStorage.getItem("s_orderEjecutive");
    if (orderby) {
      setOrderBy(JSON.parse(orderby));
    }

    let asc = localStorage.getItem("s_ascEjecutive");
    let newVariable = localStorage.getItem("saleNew_ascEjecutive");

    if (newVariable === null) {
      localStorage.setItem("s_ascEjecutive", JSON.stringify(false));
      localStorage.setItem("saleNew_ascEjecutive", JSON.stringify(ASC));
      setASC(false);
    } else {
      if (asc) {
  ...
  const getOportunitiesSales = async () => {
    ...
    try {
      let params = {
        ...
        limit: limit,
        count: 1,
        join: "ph,prospect,prospect.origin,prospect.category,prospect.clientcompa",
        order: validateOrder(),
        skip: page,
        // showlabel: "1",
      };
      ...
      saveInLocalStorage(filters, "s_FiltersExecutive");
      saveInLocalStorage(ASC, "s_ascEjecutive");
      saveInLocalStorage(orderBy, "s_orderEjecutive");
  ...
  const deleteDataSale = async reasonMessagge => {
    try {
      let response = await api.put(`oportunities/resetsaletooportunity/${dataSaleSelected.id}`);
      createTracking(dataSaleSelected.id, dataSaleSelected.prospectId, dataSaleSelected.saleData, reasonMessagge);
  ...
  const createTracking = (idOpor, idProsp, sale, reasonMessagge) => {
    try {
      let bodyNewTracking = {
        prospectId: idProsp,
        status: "2",
        ...
        observations: `Se elimina Venta: Concepto ${sale.concept}, Monto Total ${formatNumber(
          ...
          sale.quantity
        }, eliminada por ${userData.name} ${userData.lastname}, Razón:  ${reasonMessagge}`,
  ...
  return (
    <SalesStyled>
      ...
      <FiltersSales anchor="right" open={showFilters} onClose={closeDrawerFilters}>
