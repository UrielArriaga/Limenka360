...
import { Button, Dialog, Divider, Menu, Popover, Tooltip } from "@material-ui/core";
import {
  AddAlert,
  AttachMoney,
  CloseOutlined,
  ...
  switch (headName) {
          ...
          <div className="content ">
            ...
            <div className="content__more">
              <p className="txt-lastracking">
                Ultima actualización <span>{getDiferrenceDates(itemData.updatedAt)} </span>
              </p>
              <p className="txt-createdAt">
                Creado <span>{getDiferrenceDates(itemData.createdAt)} </span>
              </p>
            </div>
          </div>
        </TableDataId>
    ...
    case "estado":
      return (
        <TableData isPar={isPar} isNew={isNew}>
          <div className="content">
            {item == "Aprobado" ? (
              <p className="ok">{item}</p>
            ) : item == "Rechazado" ? (
              <p className="rejected">{item}</p>
            ) : item == "Pendiente de aprobación" ? (
              <p className="pending">{item}</p>
            ) : (
              <p>{item}</p>
            )}
          </div>
    ...
    case "factura":
      return (
        <TableData isPar={isPar} isNew={isNew}>
          <div className="content">
            <p className="phase-title">{item ? "Con factura" : "Sin factura"}</p>
          </div>
        </TableData>
      );
    ...
    case "descartado":
      return (
        <TableData isPar={isPar} isNew={isNew}>
          <div className="content">
            <p className="phase-title">{item ? "Descartado" : "Sin Descartar"}</p>
          </div>
        </TableData>
      );
    ...
    case "estado logistica":
      return (
        <TableData isPar={isPar} isNew={isNew}>
          <div className="content">
            {(item === "" || item === null || item === undefined) && <p className="notasigned">N/A</p>}
            {item !== "" && <p>{item === "pedido nuevo" ? "Proceso no Iniciado" : item}</p>}
          </div>
        </TableData>
      );
...
const TableComponent = props => {
  ...
  const saveAndRetrivelocalStorage = () => {
    try {
      ...
      let savedHeads = localStorage.getItem("tableOrderAdministration");
      if (savedHeads === null || savedHeads === undefined) {
        setHeadsTable(normalizeHeads);
        localStorage.setItem("tableOrderAdministration", JSON.stringify(normalizeHeads));
      } else {
  ...
  const updateLocalStorageOnFirstLoad = () => {
    // Check if it's the first load
    const isFirstLoad = localStorage.getItem("firstLoad") === null;

    if (isFirstLoad) {
      const updatedHeadsTable = headsTable.map(item => {
        if (item.headTextData === "forma de pago" || item.headTextData === "Metodo de Pago") {
          // Hide forma de pago and Metodo de Pago by default
      ...
      // Update localStorage
      localStorage.setItem("tableOrderAdministration", JSON.stringify(updatedHeadsTable));
  ...
  let normalizeHeads = heads.map((item, index) => ({
    headText: item,
    showColumn: true,
    showColumn: item === "id" || item === "forma de pago" || item === "Metodo de Pago" ? false : true,
    position: index,
    headTextData: item,
  }));
  ...
  const handleFuctionOptions = rowSelected => {
                      ...
                      >
                        <div className="options">
                          <div
                            ...
                            onClick={() => {
                              {
                                rowSelected.data.orderstatus.status == 3
                                  ? handleFuctionOptions(rowSelected)
                                  : handleFuctionOptions(rowSelected);
                              }
                            }}
                          >
                          ...
                          {rowSelected?.estado !== "Aprobado" && (
                            <>
                              {approveOrder && (
                                <div
                                  className="options__option"
                                  onClick={() => {
                                    approveOrder(rowSelected);
                                  }}
                                >
                                  <ThumbUpIcon />
                                  <p>Aprobar</p>
                          ...
                          {rowSelected?.estado !== "Rechazado" && (
                            <>
                              {rejectOrder && (
                                <div
                                  className="options__option"
                                  onClick={() => {
                                    rejectOrder(rowSelected);
                                  }}
                                >
                                  <ThumbDownIcon />
                                  <p>Rechazar</p>
...
import { formatDate, handleGlobalAlert, toUpperCaseChart } from "../../../../utils";
...
const ModalSettings = ({
  ...
  const handleCloseSettings = () => {
    setOpen(!open);
    setFlag(!flag);
    setOrdersFetched(!ordersFetched);
  };
  ...
  const saveUpdatesTable = data => {
    localStorage.setItem("tableOrderAdministration", JSON.stringify(data));
  };
