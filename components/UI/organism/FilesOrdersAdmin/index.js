...
export default function FilesOrdersAdmin({ idOrder }) {
    ...
    <Filess container spacing={3}>
      {isLoaderFiles ? (
        ...
        files?.map((item, index) => (
          <div className="filesItem" key={index}>
            <div className={`fileData ${styleTypeFile(item)}`}>
              ...
              <Tooltip title={"Ver Vista Previa" + " " + item.name} arrow>
                <p className="nameFile" onClick={() => handleFile(item)}>
                  {item.name ? toUpperCaseChart(item.name) : "Sin Nombre"}
                </p>
              </Tooltip>
      ...
      <PreviewStyled open={openPreview} onClose={handleClosePreviewFile} anchor="right">
