...
const TableComponentOportunitites = props => {
  const { heads, id, secondaryColor, primaryColor, data, discartedTable } = props;
  const { handleClickName, handleClickAddTracking, handleClickAddPending, handleClickReasingProspect } = props;
  ...
  const saveAndRetrivelocalStorage = () => {
    try {
      ...
      let savedHeads = localStorage.getItem("tableReportsOriginsOportunities");
      if (savedHeads === null || savedHeads === undefined) {
        setHeadsTable(normalizeHeads);
        localStorage.setItem("tableReportsOriginsOportunities", JSON.stringify(normalizeHeads));
      } else {
...
export default TableComponentOportunitites;
...
const ModalSettings = ({ open, setOpen, headsTable, setHeadsTable, id, heads }) => {
  ...
  const saveUpdatesTable = data => {
    localStorage.setItem("tableReportsOriginsOportunities", JSON.stringify(data));
  };
