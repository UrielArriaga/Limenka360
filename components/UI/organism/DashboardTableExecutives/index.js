...
export default function DashboardTableExecutives(props) {
  const dispatch = useDispatch();
  const { heads, id, secondaryColor, primaryColor, data, refetch, setRefetch, isLoading, type } = props;
      ...
      case "bysales":
        return (
          <TableBody>
            {data.map((item, index) => {
              return (
                <TableRowBody key={index} isPar={index % 2 == 0} onClick={() => dispatch(setEjecutiveSelectedG({ item }))}>
