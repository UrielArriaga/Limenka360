...
import { url_filesCompanies } from "../../services/api";
import { validateURL } from "../../utils";
const TableCustomGroups = ({
  heads,
  data,
  identificador,
  deleteItem,
  ...
  return (
    <StyledTableCustomGroups primaryColor={primaryColor} secondaryColor={secondaryColor}>
                          ...
                          <td className="data" key={ix}>
                            {typeof cell[ix] !== "object" && (
                              <div className="ctr_td">
                                {keyCell[ix]?.includes("color primario") ||
                                  (keyCell[ix]?.includes("primarycolor") && (
                                    <input className="inputColor" type="color" value={cell[ix]} disabled />
                                  ))}
                                {keyCell[ix]?.includes("color secundario") ||
                                  (keyCell[ix]?.includes("secondarycolor") && (
                                    <input className="inputColor" type="color" value={cell[ix]} disabled />
                                  ))}
                                {!cell[ix] ? (
                                  <span>N/A</span>
                                ) : keyCell[ix]?.includes("logo") ? (
    ...
    </StyledTableCustomGroups>
  );
};

export default TableCustomGroups;

const StyledTableCustomGroups = styled.div`
  width: 100%;
  position: relative;
