...
export default function AddPendingCalendary({
  data,
  prospect,
  close,
  open,
  ...
  function formatnewDate(str) {
    let newDateFormated = new Date(str);
    return newDateFormated.toISOString();
  }
  ...
  const handleAddPending = async formData => {
    if (isType === true) {
      ...
      try {
        ...
        newPending.prospectId = prospect?.prospectId;
        newPending.status = "1";

        if (data?.prospect?.itemBD?.isoportunity == true) {
          newPending.oportunityId = prospect?.prospectId;
          newPending.status = "2";
          newPending.prospectId = prospect?.prospectId;
        }
        ...
        if (addPending.status == 201) {
          ...
          let trackingPending = {};
          //se modificados los status por banderas 10/11/2022
          trackingPending.prospectId = prospect?.prospectId;
          ...
          if (data?.isCloseOut == true) {
            trackingPending.prospectId = prospect?.prospectId;
            trackingPending.oportunityId = prospect?.prospectId;
