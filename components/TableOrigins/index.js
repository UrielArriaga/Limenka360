...
const OriginsTable = ({ totalOrigins, setTotalOrigins, searchKey, filterOrigin, flag }) => {
  ...
  const getData = async () => {
    try {
      ...
      let origin = await api.get(`origins?${queryString}&${count}&${limite}&${skip}&${order}`);
  ...
  const handleUpdateData = formData => {
    ...
    api.put(`origins/${dataToUpdate.id}`, jsonUpdate).then(res => {
      console.log(res);
      setRefetch(!refetch);
      resetForm();
      handleClose();
  ...
  const handleDeleteData = () => {
    api.delete(`origins/${dataToDelete.id}`).then(res => {
      setRefetch(!refetch);
      handleCloseDelete();
    });
  };
  ...
  const handleDeleteMany = () => {
    dataToManyDelete.forEach(item => {
      api.delete(`origins/${item.id}`).then(res => {
        setShowDeleteMany(false);
  ...
  return (
    <TableOrigins>
      ...
      >
        <DialogContainer>
          <p className="title">Eliminar Origen {dataToDelete?.name}</p>
