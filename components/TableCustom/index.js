                          ...
                          <td className="data" key={ix}>
                            {typeof cell[ix] !== "object" && (
                              <div className="ctr_td">
                                {keyCell[ix]?.includes("phone") ||
                                  (keyCell[ix]?.includes("movil") && (
                                    <Smartphone fontSize="small" className="icon_phone" />
                                  ))}
                                {keyCell[ix]?.includes("telefono") && cell[ix] !== "" && (
                                  <CallOutlined fontSize="small" className="icon_telefono" />
                                )}
                                {keyCell[ix]?.includes("fecha") && cell[ix] !== "" && (
                                  <Event fontSize="small" className="icon_date" />
                                )}

                                {keyCell[ix]?.includes("nameprospect") && cell[ix] !== "" && (
                                  <Person fontSize="small" className="icon_person" />
                                )}
                                {keyCell[ix]?.includes("ejecutive") && cell[ix] !== "" && (
                                  <Person fontSize="small" className="icon_person" />
                                )}
                                {keyCell[ix]?.includes("monto") ||
                                  (keyCell[ix]?.includes("amount") && (
                                    <AttachMoney fontSize="small" className="icon_phone" />
                                  ))}
                                {keyCell[ix]?.includes("color ") ||
                                  (keyCell[ix]?.includes("color") && (
                                    <input className="inputColor" type="color" value={cell[ix]} disabled />
                                  ))}
                                {!cell[ix] === undefined ? (
                                  <span>N/A</span>
                                ) : keyCell[ix]?.includes("photo") ? (
                                  <Avatar variant="rounded" className="avatar" src={`${validateURL(cell[ix])}`} />
                                ) : (
                                  cell[ix]
                                )}
                              </div>
