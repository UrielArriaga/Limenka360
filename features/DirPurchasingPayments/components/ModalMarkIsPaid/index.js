...
import { ButtonContainer, StyledBox } from "./styles";

const ModalMarkAsPaid = ({ open, handleClose, item, refresh, setSelectedPay }) => {
  ...
  const pay = async () => {
    ...
    if (dataToUpdate.paymentwayId != "" && dataToUpdate.paymentaccountId != "") {
      ...
      if (update.status === 200) {
        setSelectedPay(prevState => ({...prevState, pagado:"Pagado"}));
        handleClose();
  ...
  return (
    <Modal open={open} onClose={handleClose} aria-labelledby="modal-title" aria-describedby="modal-description">
      <StyledBox>
        <Typography className="titleModal" id="modal-title" variant="h6" component="h2">
          Marcar como pagado
        </Typography>
        <div className="item">
        ...
        <ButtonContainer>
          <Button className="btnCancel" onClick={handleClose} variant="contained">
            Cancelar
          </Button>
          <Button className="btnAdd" onClick={pay} variant="contained">
            Confirmar
          </Button>
        </ButtonContainer>
      </StyledBox>
