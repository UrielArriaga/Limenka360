...
export default function usePurchasingManagerPedidos(activeFilters, isAdmin = false, folio = "") {
  ...
  const {
    page: pagePro,
    limit: limitPro,
    handlePage: handlePaginationPro,
  } = usePagination({ defaultLimit: 20, defaultPage: 1 });
  ...
  const getOrders = async () => {
    try {
      setOrdersActives({ ...ordersActives, isFetchingData: false });
      let response = await ordersService.getOrdersActives(limit, page);
      if (response.status == 200) {
        setOrdersActives({ data: response.data.results, total: response.data.count, isFetchingData: false });
      }
    } catch (error) {
      setOrdersActives({ ...ordersActives, isFetchingData: false });
    }
  ...
  const deleteKeyWord = () => {
    setKeyword(""), router.replace(router.pathname, undefined, { shallow: true });
  };
