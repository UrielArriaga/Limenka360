...
import usePurchasingManagerPedido from "./hooks/usePurchasingManagerPedido";
import usePurchasingManagerPedidos from "./hooks/usePurchasingManagerPedidos";
...
export default function PurchasingManagerPedidos({ isAdmin = false }) {
  ...
  const {
    ...
    tabValue,
    handleOnKeyEnter,
  } = usePurchasingManagerPedidos(activeFilters, isAdmin, folio);

  const { paginationData: paginationProviders, providers } = providersData;

  const { orderSelectedData, isFetchingOrder, productsData, totalOrdersShopping, getDataOrder } =
    usePurchasingManagerPedido(orderSelected);
  const {
    openTrackings,
