...
import useGenerateDocs from "./hooks/useGenerateDocs.js";

const DepAttendantNewRoute = () => {
  ...
  const { handleEditProduct,generarFolio } = useCartPort(ordersToAdd);

  const { shippingToOrders, isPackage } = useAddShippingToOrders(ordersToAdd);

  useEffect(() => {
    console.log("ord", ordersToAdd);
  }, [ordersToAdd]);
...
export default DepAttendantNewRoute;
