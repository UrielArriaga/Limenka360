  ...
  const handleEditProduct = (value, identifier, position, item) => {
    ...
    if (value === 0 || value === undefined || value === "" || value === undefined) {
      ...
      if (identifier === "quantity") {
        ...
        iva = subtotal * 0.16;
        total = value * item.unitprice + iva;
      } else if (identifier === "unitprice") {
        subtotal = value * item.quantity;
        iva = subtotal * 0.16;
        total = value * item.quantity + iva;
      }
      copyProducts[position]["subtotal"] = subtotal;
      copyProducts[position]["totalIva"] = iva;
  ...
  const requestSavePushOrder = async bodyOrder => {
    try {
      ...
      if (response.status == 201) {
        ...
        createOrderDocument(response.data);
      }
    } catch (error) {
