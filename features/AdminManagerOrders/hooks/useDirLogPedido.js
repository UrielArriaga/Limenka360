...
export default function useDirLogPedido(orderSelected) {
  ...
  const normDataResults = dataResults => {
    let arrayPackage = [];
    let newData = [];
    ...
    dataResults.map(item => {
      if (!item.productpackageId) {
        newData.push(item.product.ispackage === true ? { ...item, inPackage: arrayPackage } : item);
      }
    });
  ...
  const handleOnClickSave = (productOportunitySelected, wereHouseSelected, quantity) => {
    ...
    setProducts({ ...productsData, results: normDataResults(products) });
