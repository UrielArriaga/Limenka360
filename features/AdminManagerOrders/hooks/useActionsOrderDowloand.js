...
export const useActionsOrderDowloand = (orderSelected, updatePropertyLocal, updateItemPropertyLocal, productData) => {
  ...
  const formatEjecutive = (roleId, userData, infoOrders) => {
    ...
    return {
      name: isExecutive
        ...
        : infoOrders?.oportunity?.soldby?.name,
      lastname: isExecutive ? toUpperCaseChart(userData?.lastname) : infoOrders?.oportunity?.soldby?.lastname,
      phone: isExecutive ? userData?.phone : infoOrders?.oportunity?.soldby?.phone,
    };
  };
  ...
  const generateAndDowloandPDF = async () => {
    try {
      ...
      formData.append("data", makeTemplateOrder(data));
