...
export default function useDirLogFiles(orderSelected) {
  ...
  useEffect(() => {
    if (!orderSelected) {
      console.error("orderId no recibido");
      return;
    }
    console.log("orderId recibido en hook:", orderSelected);
  }, [orderSelected]);

  useEffect(() => {
    console.log("orderId es:", orderSelected);
  ...
  const createParams = () => ({
    where: JSON.stringify({ orderId: orderSelected }),
    include: "filestype,uploadby,order",
    join: "filestype,das,order",
    limit: limit,
    skip: page,
    count: 1,
    order: "-createdAt",
  ...
  const handleOnClickUploadFiles = async () => {
    try {
      for (let i = 0; i < filesToUpload.length; i++) {
        ...
        await api.post(`files/fileuploaddata/${orderSelected}`, formDataFile, {
  ...
  const funtionSeleccion = item => {
    console.log("item--->",item);
    console.log("el arreglo",filesData?.results);
    let arrayFiles = filesData?.results.filter((data) => data.orderId === item.orderId)
    console.log("filtrito de archivos", arrayFiles);
    handleToggleFiles()
  }
