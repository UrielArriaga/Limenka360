...
class DirLogInventaryUnitService {
  ...
  normalizeProductsInventory(productIntentory) {
    ...
    return {
      ...
      entrydate: productIntentory?.inventoryentry?.createdAt
        ? dayjs(productIntentory?.inventoryentry?.createdAt).format("DD/MM/YYYY")
        : "No aplica",
      outputdate: productIntentory?.inventoryexit?.createdAt
        ? dayjs(productIntentory.inventoryexit.createdAt).format("DD/MM/YYYY")
        : "No aplica",
      warehouse: productIntentory?.warehouse?.name || "No aplica",
      warehouseId: productIntentory?.warehouseId || null,
      ...
      reviewed:
        productIntentory?.reviewed === false
          ? "No revisado"
          : productIntentory?.reviewed === true
          ? "Revisado"
          : "Estado desconocido",
      intransit: productIntentory?.intransit ? "En transito" : "No Aplica",
      stockintransit: productIntentory?.product?.stockintransit,
