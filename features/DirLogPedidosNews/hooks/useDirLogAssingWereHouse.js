...
export default function useDirLogAssingWereHouse(orderSelected, productsData) {
  const { open: openWereHouses, toggleModal: handleToggleWereHousesModal } = useModal();
  ...
  const handleToggleWereHouses = () => {
    if (openWereHouses) {
      handleToggleWereHousesModal();
      return;
    }
    let confirm = window.confirm("Asegurate de no tener productos asignados");
    if (confirm) {
      handleToggleWereHousesModal();
    }
  };
