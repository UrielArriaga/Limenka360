...
export default function PreviewOrder({
  ...
  // console.log(paginationDatas, " paginations");
  ...
  const getMenuOptions = () => {
    return [
      { label: "Eliminar", action: handleDeleteArticle },
      { label: "Apartar Articulo", action: handleArticle },
      { label: "Quitar de Apartado", action: handleIsNotApart },
    ];
  };
  return (
    <PreviewOrderStyled>
      ...
      <div className="contentpreview">
                  ...
                  <div key={index} className={`tablerow-container ${index % 2 === 0 ? "even-row" : "odd-row"}`}>
                    ...
                    <div className="tablerow">
                      ...
                      <div className="tablecell actions">
                        <div>
                          {/* <pre>{JSON.stringify(productOportunity?.isComplete)}</pre> */}

                          {productOportunity?.totalorder === productOportunity?.total ? (
                            // <IconButton className="disabled" disabled={true}>
                            <div
                              style={{
                                display: "flex",
                                alignItems: "center",
        ...
        <div className="pagination">
          <Pagination
            count={Math.ceil(paginationDatas.total / paginationDatas.limitResults)}
            onChange={(e, value) => paginationDatas.handlePage(value)}
            page={paginationDatas.pageCurrent}
            size="small"
            color="primary"
          />
        </div>
