...
import { ORDERSTATUS, typeSockets } from "../../constants";
...
export default function AdminAlmacenPedidos({ isAdmin = true }) {
  ...
  // * LOGIC PEDIDO
  const { orderSelectedData, isFetchingOrder, productsData, ShippingsData } = useDirLogPedido(orderSelected);
...
function useProductsToExit(orderSelected, refetchData, max) {
  ...
  const handleAddArticleToProduct = (product, article, isChecked) => {
    let articlefinal = {
      ...article,
      comments: "",
      warehouseOrderId: product?.id,
    };

    let newArray = products?.results.map(item => {
      if (product?.id === item?.id) {
        if (isChecked) {
          // Agregar artículo si isChecked es true
          return {
            ...item,
            articles: item?.articles ? [...item.articles, articlefinal] : [articlefinal],
          };
        } else {
          // Eliminar artículo si isChecked es false
          return {
            ...item,
            articles: item?.articles ? item.articles.filter(art => art.id !== article.id) : [],
          };
    ...
    const updatedProduct = newArray.find(item => item.id === product.id);
    setProductToSearch(updatedProduct);

    // Si necesitas actualizar el contador de artículos seleccionados
  ...
  const handleOnClickNewExit = async () => {
    ...
    try {
      ...
      pushNotification(typeSockets.new_warehouse_tosupply.value, {
