...
import useReplaceProducts from "../../hooks/useReplaceProducts.js";
import DrawerReplaceProduct from "../DrawerReplaceProduct/index.js";
...
export default function PreviewOrder({
  ...
  const getMenuOptions = () => {
    return [
      { label: "Cambiar Estatus", action: handleRejectOrder },
    ];
  };
  ...
  const replaceProducts = useReplaceProducts();

  const handleConfirmReplace = async (newProduct) => {
    const success = await replaceProducts.createReplace( productToReplace, newProduct, );
    if (success) {
      replaceProducts.handleCloseModal();
  ...
  return (
    <PreviewOrderStyled>
        ...
        <div className="contentpreview__containerTable">
          ...
          <div className="contentpreview__products">
            <ProductTable
              ...
              setSelectAll={setSelectAll}
              setSelectedProvider={setSelectedProvider}
              onOpenReplaceDrawer={(product) => {
                setProductToReplace(product);
                replaceProducts.handleOpenModal();
              }}
            />
          </div>
      ...
      <DrawerReplaceProduct
        open={replaceProducts.open}
        onClose={replaceProducts.handleCloseModal}
        products={replaceProducts.products}
        paginationData={{
          page: replaceProducts.paginationData.page,
          limit: replaceProducts.paginationData.limit,
          handlePagination: replaceProducts.paginationData.handlePagination
        }}
        keyWord={replaceProducts.keyWord}
        handleSearchProduct={replaceProducts.handleSearch}
        handleClickAddProduct={handleConfirmReplace}
        fetchAllProducts={replaceProducts.fetchAllProducts}
        originalProduct={productToReplace}
      />
    </PreviewOrderStyled>
  );
}
