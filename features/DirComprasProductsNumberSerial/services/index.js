...
export class ProductsServices {
  async getProducts(limit = 20, page = 1, orderBy, query) {
    let params = {
      include: "product,purchaseorder,purchaseorder.statuspoo",
      // join: "pro",
      limit: limit,
      skip: page,
      count: 1,
    ...
    return await api.get("warehouseproducts", {
      params,
    });
  }
  ...
  normalizeDataOrders(item) {
    ...
    return {
      id: item.id,
      serial: item?.serialnumber || "S/N",
      code: item?.product?.code || "S/C",
      name: `${toUpperCaseChart(item?.product?.name)}` || "N/A",
      // stock: item?.stock ,
  ...
  async postWarranty(body){
    return await api.post(`warrantywarehouseproduct`, body);
  }
}
