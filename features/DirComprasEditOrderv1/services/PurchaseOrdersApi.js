...
export class PurchaseOrdersApi {
  ...
  async putOrder(id, data) {
    return await api.put(`purchaseorders/${id}`, data);
  }
  ...
  async normalizeOrderData(object) {
    let { form, products, payments, id_user, addressSelected, userData, dataPurchase } = object;
    ...
    return {
      folio: dataPurchase?.folio || `${userData?.username}${"OR"}-${responseCount}`,
  ...
  async getPurchaseById(orderId){
    return await api.get(`purchaseorders/${orderId}`);
  }
}
