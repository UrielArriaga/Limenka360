...
import { NewOrderStyle } from "./styles";
...
import DrawerSerialNumbers from "./components/DrawerSerialNumbers";
import useOrderData from "./hooks/useOrderData";

export default function DirComprasEditOrderv1() {
  const { paymentsControl } = usePayments({ viewControl });
  const { productControl } = useProducts({ viewControl });

  const { viewControl, formControls, catalogs, modalControl, provider, setStates, handleBack } = useMain({
    productControl,
    paymentsControl,
  });
  ...
  const views = {
    buyerData: <FormBuyer formControls={formControls} />,
    orderData: <FormOrder formControls={formControls} catalogs={catalogs} />,
    products: (
      <FormProducts
        provider={provider}
    ...
    payments: <FormPayments formControls={formControls} paymentsControl={paymentsControl} catalogs={catalogs} />,
    resume: (
      <Resume
        formControls={formControls}
        productControl={productControl}
        type={viewControl.type}
        previewData={viewControl.previewData}
      />
    ),
  };
  ...
  return (
    <NewOrderStyle>
      <div className="flex-aling-center header">
        <IconButton className="bt_back" onClick={() => handleBack()}>
          <ArrowBack />
        </IconButton>
        <h1>Edicion de orden de compra</h1>
      </div>
      ...
      <div className="main_order">
        ...
        <div className="data">{views[viewControl.view] || null}</div>
        ...
        <div className="fixed">
          <div className="flex-center-end">
            <Button
              ...
              disabled={viewControl.isCreating}
              onClick={formControls.handleSubmit(formControls.onSubmit(true))}
            >
            ...
            <Button
              ...
              className="btn_generate"
              disabled={viewControl.isCreating}
              onClick={formControls.handleSubmit(formControls.onSubmit(false))}
            >
              Guardar Cambios(finalizar)
            </Button>
          </div>
        </div>
      </div>
      ...
      <AddNewProduct
        open={modalControl.openAddProduct}
        close={modalControl.handleToogleAddProduct}
        provider={provider}
        addProduct={productControl.handleOnClickAddProduct}
        // open={openAddProduct}
      ...
      <DrawerSerialNumbers
        productControl={productControl}
        open={productControl.isOpenSerialNumbers}
        handletoogle={productControl.closeModalNumbers}
        serialNumbersbyProduct={productControl.serialNumbersbyProduct}
        addSerialNumers={productControl.addSerialNumers}
      />
    </NewOrderStyle>
  );
}
...
// export default function DirComprasNewOrder() {
//   const {
//     showCalendary,
//     setShowCalendary,
//     payments,
...
//     roleId == "compras_internacional" ? [{ name: "Internacional", id: false }] : [{ name: "Nacional", id: true }];
