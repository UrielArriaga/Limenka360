...
export class ShippingsOrdersServices {
  ...
  async getPaymentsPurchase(query) {
    let params = {
      include: "purchaseorder,conceptimport",
      count: 1,
      where: JSON.stringify(query),
    };
  ...
  async putPaymentsPurchase(data, id) {
    console.log("Data que se va a enviar:", data);
    let body = {
      payments: data,
    };
    return await api.put(`paymentspurchaseorder/${id}`, body);
  }
  async putPaymentss(id, payment) {
    ...
    return  await api.put(`paymentspurchaseorder/`, body);
  }
  

  async getPayments(id) {
    return await api.get(`paymentspurchaseorder/${id}`);
  }
}
