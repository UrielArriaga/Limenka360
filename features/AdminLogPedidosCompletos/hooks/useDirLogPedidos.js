...
export default function useDirLogPedidos(activeFilters, isAdmin = false, status, setActiveFilters) {
  ...
  const getData = async removeKeyword => {
    try {
      ...
      if (folioParamUrl) {
        let ordercomplete = normalizeData?.find(item => item.folio == folioParamUrl);
        if (ordercomplete) handleOnClickRow(ordercomplete);
      }
    } catch (error) {
      console.log(error);
      setIsFetchingData(false);
  ...
  const deleteKeyWord = () => {
    ...
    delete router?.query;
    getData(true);
    if (folioParamUrl) router.push("/administracionlogistica/pedidoscompletos");
  };
