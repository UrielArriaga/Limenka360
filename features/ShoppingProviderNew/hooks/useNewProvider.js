...
export default function useNewProvider(allContact) {
  ...
  const handleCreateProspect = formData => {
    ...
    const newProvider = {
      ...
      national: formData?.national ? true : false,
  ...
  const PostProvier = async item => {
    ...
    try {
      ...
      if (providerNew.status == 201) {
        showAlertSucces("Se agrego el proveedor correctamente");
      ...
      showAlertError("Ocurrio un error al agregar el proveedor" + idError);
      saveError(error, idError, "newprovider");
    }
  };
