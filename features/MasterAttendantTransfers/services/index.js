...
export class OrdersServices {
  ...
  async getProductsOfTransfers(query) {
    let params = {
      where: JSON.stringify(query),
      include:
        "warehouseproduct,warehouseproduct.product,warehouseproduct.inventoryentry,warehouseproduct.inventoryexit",
    };
  ...
  normalizeDataTransfers(item) {
    return {
      ...
      entrydate: item?.warehouseentry ? dayjs(item?.warehouseentry).format("D,MMMM  YYYY") : "N/A",
      exitdate: item?.warehouseexit ? dayjs(item?.warehouseexit).format("D,MMMM  YYYY") : "N/A",
      exitwarehouse: item?.exitwarehouse?.name,
      entrywarehouse: item?.entrywarehouse?.name,
    };
  }

  async postExitTransfer(body) {
    return await api.post(`inventoryexits/transfer`, body);
  }
  async createPDF(form) {
    return await api.post("convert/pdf", form);
  }
  async putFormatTransfer(id, body) {
    return await api.put(`inventorytransfers/${id}`, body);
  }
  async postEntryTransfer(body) {
    return await api.post(`inventoryentries/transfer`, body);
  }
}
