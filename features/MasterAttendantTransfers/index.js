import { MasterAlmacenStyled } from "./styles";
...
import useMasterAlmacenTransfers from "./hooks/useMasterAlmacenTransfers";
import useFilters from "./hooks/useFilters";
import useMasterAlmacenTransferProducts from "./hooks/useMasterAlmacenTransferProducts";
import ModalExit from "./components/ModalCreateExit";
import ModalEntry from "./components/ModalCreateEntry";
import useMasterAlmacenTransferProductsEntry from "./hooks/useMasterAlmacenTransferProductsEntry";

function MasterAttendantTransfers({ isAdmin = false, status }) {
  ...
  const {
    ...
    lastFetchDate,
    selectedTransfer,
  } = useMasterAlmacenTransfers(activeFilters, isAdmin, status, setActiveFilters, setproductoportunityorder);
  ...
  const {
    ...
    setObservations,
    handleOnSaveExit,
  } = useMasterAlmacenTransferProducts(selectedTransfer);

  const {
    createEntryArticles,
    productsTransfer:transferProduct,
    isFetching:fetching,
    openModalEntry,
    toggleModalEntry,
    closeModalEntry,
    handleOnSaveEntry:onSaveEntry,

  } = useMasterAlmacenTransferProductsEntry(selectedTransfer);
  ...
  return (
    <MasterAlmacenStyled>
                ...
                <div style={{ overflow: "hidden" }}>
                  ...
                  >
                    <PreviewOrder
                      ...
                      createExitArticles={createExitArticles}
                      createEntryArticles={createEntryArticles}
                    />
                  </motion.div>
                </div>
              </AnimatePresence>
      ...
      <ModalExit
        ...
        handleOnSaveExit={handleOnSaveExit}
      />
...
export default MasterAttendantTransfers;
