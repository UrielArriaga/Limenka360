...
export default function useFloorManagerPedidos(activeFilters, inventoryExitId) {
  ...
  const buildQuery = () => {
    let query = {
      // exitstatus: ORDERSTATUS.completado,
      $or: [{ exitstatus: "completado" }, { exitstatus: "entregado" },{ exitstatus: "parcialmente surtido" }],
    };
  ...
  const getData = async (removeKeyword) => {
    try {
      ...
      let query = {
        ...
        order: inquery,
        statuswho: "completado",
      };

      if (!removeKeyword && keyword && keyword.length > 2) {
        inquery.folio = { iRegexp: keyword.trim() };
      }
      if (!removeKeyword && folioParamUrl ) {
        inquery.folio = { iRegexp: folioParamUrl };
  ...
  const deleteKeyWord = () => {
    ...
    delete router?.query;
    getData(true);
    if(folioParamUrl) router.push("/encargadosalidas/pedidoscompletados")
  }
