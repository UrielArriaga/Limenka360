...
export default function useFloorManagerFiles(orderSelected) {
  ...
  const createParams = () => ({
    where: JSON.stringify({ warehouseproductId: articleSelected.id }),
  ...
  const handleOnClickUploadFiles = async () => {
    try {
      for (let i = 0; i < filesToUpload.length; i++) {
        ...
        await api.post(
          // `files/fileuploaddata/${orderSelected?.itemData?.orderId}/${replaceSpacesWithHyphens(filesToUpload[0].name)}`,
          `files/fileuploaddata/${orderSelected?.itemData?.orderId}`,
          formDataFile,
          {
