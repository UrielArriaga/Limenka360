...
export default function useAdmAlmacen(activeFilters, isAdmin = false) {
  ...
  const buildQuery = () => {
    let query = {
      exitstatus: {
        $in: [ORDERSTATUS.surtir, ORDERSTATUS.surtido] 
      },
    };
  ...
  const getData = async removeKeyword => {
    try {
      ...
      if (!removeKeyword && keyword && keyword.length > 3) {
        inquery.folio = `${keyword}`;
      }
      if (!removeKeyword && folioParamUrl) {
        inquery.folio = `${folioParamUrl}`;
  ...
  const handleOnClickFloorManager = async () => {
    try {
      let resp = await ordersService.updateOrderStatus(orderSelected.orderId, "en piso");
      showAlertSucces("Pedido marcado como en piso");
      getData();
    } catch (error) {
  ...
  const deleteKeyWord = () => {
    ...
    delete router?.query;
    getData(true);
    if (folioParamUrl) router.push("/almacenesforaneos");
  };
