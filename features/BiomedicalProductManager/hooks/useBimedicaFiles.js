...
export default function useBimedicaFiles(productSelect) {
  ...
  useEffect(() => {
    if (productSelect && openFiles) {
      fetchFiles();
    }
  }, [productSelect, refetch, page, openFiles]);
  ...
  const createParams = () => ({
    where: JSON.stringify({ warehouseproductId: productSelect.id }),
  ...
  const handleUpdateFile = async () => {
    try {
      await api.put(`documents/${fileToUpdate.id}`, {
        ...
        filestypeId: fileToUpdate.idTypeFile,
      });
  ...
  const handleOnClickUploadFiles = async () => {    
    try {
      for (let i = 0; i < filesToUpload.length; i++) {
        ...
        formDataFile.append("warehouseproductId", productSelect?.id);
        formDataFile.append("uploadbyId", id_user);
        formDataFile.append("filestypesId", filesToUpload[i].idTypeFile);
        await api.post(
          `files/fileuploaddatawarehouseproductId/${productSelect?.id}`,
          formDataFile,
          {
  ...
  const handleOnChangeEditFile = (index, key, value) => {
    ...
    if(key === "name"){
      ...
      let f = newFiles[index]["file"];
      let file = new File([f],value)
