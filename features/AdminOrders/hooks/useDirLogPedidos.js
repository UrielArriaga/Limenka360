...
export default function useDirLogPedidos(activeFilters, setActiveFilters, orderStatus) {
  ...
  const buildQuery = () => {
    let query = {};
    if (orderStatus == "Rechazado") query.orderstatus = { name: "Rechazado" };
    if (orderStatus == "Pendiente de aprobación") query.orderstatus = { name: "Pendiente de aprobación" };
    if (orderStatus == "Cancelado") query.orderstatus = { name: "Cancelado" };
