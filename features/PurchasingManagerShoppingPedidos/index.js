...
import usePurchasingManagerShoppingPedido from "./hooks/usePurchasingManagerShoppingPedido";
import usePurchasingManagerShoppingPedidos from "./hooks/usePurchasingManagerShoppingPedidos";
...
export default function PurchasingManagerPedidos({ isAdmin = false }) {
  ...
  } = usePurchasingManagerShoppingPedidos(activeFilters, isAdmin, folio);

  const { orderSelectedData, isFetchingOrder, productsData, totalOrdersShopping, getDataOrder } =
    usePurchasingManagerShoppingPedido(orderSelected);
  const {
    openTrackings,
