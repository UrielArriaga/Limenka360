...
export default function useDepAttendantExitsFiles(orderSelected) {
  ...
  const handleOnClickUploadFiles = async () => {
    try {
      for (let i = 0; i < filesToUpload.length; i++) {
        ...
        await api.post(
          `files/fileuploaddata/${orderSelected?.orderId}`,
          // `files/fileuploaddata/${orderSelected?.orderId}/${replaceSpacesWithHyphens(filesToUpload[0].name)}`,
          formDataFile,
          {
