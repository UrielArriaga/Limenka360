...
export default function useShoppingFiles(orderSelected) {
  ...
  const createParams = () => ({
    ...
    include: "filestype,uploadby,purchaseorder",
    join: "filestype,das",
    limit: limit,
    skip: page,
    count: 1,
  ...
  const handleOnClickUploadFiles = async () => {    
    try {
      for (let i = 0; i < filesToUpload.length; i++) {
        ...
        let formDataFile = {};
        formDataFile.url = "";
        formDataFile.isevidence = false;
        formDataFile.name = filesToUpload[i].name;
        formDataFile.size = filesToUpload[i].size;
        formDataFile.filestypeId = filesToUpload[i].idTypeFile;
