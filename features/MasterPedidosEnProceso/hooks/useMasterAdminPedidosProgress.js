...
export default function useMasterAdminPedidosProgress(activeFilters, inventoryExitId) {
  ...
  const buildQuery = () => {
    let query = {
      exitstatus: {
        $notIn: [ORDERSTATUS.surtir, ORDERSTATUS.surtido],
      },
    };
  ...
  const deleteKeyWord = () => {
    ...
    getData(true);
    delete router?.query;
    if (folioOrderParam) router.push("/almacenesforaneos/pedidosenproceso");
  };
