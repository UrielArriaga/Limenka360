...
export default function useMasterPedidoProgress(orderSelected, handleToggleFiles, updateStateData) {
  ...
  const handleOnClickNewExit = async () => {
    ...
    try {
      ...
      console.log(" la data que manda la peticion", data);
  ...
  const updateStateProducts = (id, state) => {
    ...
    let newProductsData = { ...productsData, results: newProducts };
