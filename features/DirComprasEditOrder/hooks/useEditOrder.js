...
export default function useEditOrder() {
  ...
  const [dataPaymentsPurchase, setDataPaymentsPurchase] = useState({
    data: [],
    period: "",
    sms: "",
  });

  console.log(dataPaymentsPurchase, " data purchase");
  ...
  const handleSaveAndComplete = async form => {
    ...
    if (dataPaymentsPurchase?.sms == "Datos Incorrectos") {
      showAlertWarning("Los datos del calendario son incorrectos");
      return;
    }
  ...
  const updatePaymentsPurchase = async () => {

    let data = dataPaymentsPurchase?.data?.map(item => {
    ...
    try {
      ...
      let response = await request.putPaymentPurchaseOrder(orderId, body);
      if (response.status == 201 || response.status == 200) {
        showAlertSucces("Calendario de Pagos Actulizado");
        setLoaderSave(true);
