...
export default function useShippingOrders(activeFilters) {
  ...
  const getData = async (removeKeyword) => {
    ...
    let filteredResults = results.filter(item => item.statuspoo === "comprado");

    let normalizeData = filteredResults.map(item =>
      ordersService.normalizeDataOrdersShipping(item)
    );
    setData(normalizeData);
    ...
    if(folioParamUrl){
      ...
      if(purchase) handleOnClickRow(purchase);
    }
  } catch (error) {
    console.log(error);
    showAlertError("Ocurrio un error al obtener ordenes de compra");
  ...
  const deleteKeyWord = () => {
    ...
    setKeyword("")
    delete router?.query;
    if(folioParamUrl) router.push("/directorcompras/bandeja");
  };
