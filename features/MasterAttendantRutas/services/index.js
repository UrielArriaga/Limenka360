...
class DeliveryRoutesApi {
  ...
  normalizeDeliveryRoutes(routes) {
    return {
      id: routes?.id || "Sin id",
      data: routes,
      assigned_date: routes?.assigned_date ? dayjs(routes?.assigned_date).format("D,MMMM  YYYY") : "Sin Fecha",
      name: routes?.driver?.name || "Sin chofer",
      model: routes?.transportunit?.model || "Sin unidad",
      tuition: routes?.transportunit?.tuition || "Sin placa",
      km_output: routes?.km_output || "Sin km aún",
      km_arrival: routes?.km_arrival || "Sin km aún",
      alias:routes?.alias || "Sin Nombre"
    };
  }
}
