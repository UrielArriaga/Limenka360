...
export default function DataOrderPdf(props) {
  ...
  const { userData, id_user, roleId, companyId } = useSelector(userSelector);
  ...
  const generateAgainPdf = async () => {
    try {
      ...
      let ejecutive = {};
      if (roleId === "ejecutivo") {
        ejecutive.name = `${toUpperCaseChart(
          userData?.title
        )} ${toUpperCaseChart(userData?.name)}`;
      ...
      data.groupLogo =
        userData?.grouplogo == "" ? undefined : userData?.grouplogo;
      ...
      setIsCreatingPDF(false);
      showAlertError("Error al generar PDF");
      console.log("ERROR generateAgainPdf", error);
    }
  };
  return (
    <DataOrderPdfStyled>
