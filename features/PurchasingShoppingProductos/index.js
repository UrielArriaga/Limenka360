...
export default function PurchasingShoppingProductos() {
  ...
  const { openDrawer, selectedProduct, handleCloseEdit, handleUploadProduct, isfetching, closeDrawer } = useProductForm();

  const idEdit = selectedProduct?.selectItemProduct.id;
  const productEdit = selectedProduct?.selectItemProduct;
  ...
  const idDelete = selectedProduct?.selectItemProduct.id;
  const productDelete = selectedProduct?.selectItemProduct;
  const { deleteProduct } = useDeleteProduct( handleToggleDelete, refetchData, selectedProduct );
  ...
  return (
    <StyledProducts>
      ...
      <div className="headerTitle">
        ...
        <Button className="add" onClick={() => router.push("../gestorcompras/productos/nuevo")}>
          <Add />
          Agregar Producto
        </Button>
      </div>
