...
export default function useShippingProducts( openDrawer, setSelectItemProduct, optionsFilterSelected,  ) {
  ...
  let actions = [
    ...
    // ** Descomentar sólo si se cambia de idea para eliminar productos **//
    // {
    //   name:"Eliminar",
    //   action: e => {
    //     handleToggleDelete();
  ...
  }, [page, orderBy, keyword, optionsFilterSelected, open]);
  ...
  const getData = async () => {
    try {
      ...
      console.log(error);
      showAlertError("Ocurrió un error al obtener los productos");
