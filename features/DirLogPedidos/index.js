...
export default function DirLogPedidos({ isAdmin = false, status }) {
  ...
  const [activedModalNotification, setActivedModalNotification] = useState(false);
  ...
  } = useDirLogPedidos(activeFilters, isAdmin, status, setActiveFilters, setproductoportunityorder);
  ...
  const handleAddAllPurchaseOrders = (isChecked, products) => {
    if (isChecked) {
      setSelectedProducts(products.results?.map(product => product));
    } else {
      setSelectedProducts([]);
    }
  };
  ...
  const handleToggleProduct = productId => {
    setSelectedProducts(prevSelected => {
      if (prevSelected.includes(productId)) {
        return prevSelected.filter(id => id.id !== productId.id);
      } else {
  ...
  const handleOnClickSaveOrder = async () => {
    try {
      ...
      socket?.emit("newEvent", {
        type: typeSockets.new_order_warehouse.value,
  ...
  const { isOpen, openModal, closeModal, handleOnClickProduct, productToNotify } = useProductQuantityModal();
  ...
  return (
    <DirLogDashboardStyled>
                  ...
                  {productoportunityorder ? (
                    ...
                    >
                      <PreviewOrder
                        ...
                        activedModalNotification={activedModalNotification}
                        setActivedModalNotification={setActivedModalNotification}
      ...
      <ProductQuantityModal
        productToNotify={productToNotify}
        orderData={orderSelectedData}
        open={isOpen}
        onClose={closeModal}
        refetchPedido={refetchPedido}
