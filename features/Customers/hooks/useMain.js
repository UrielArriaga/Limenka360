...
export default function useMain(viewType) {
  ...
  const query = useMemo(() => ({ isclient: true }), []);
  ...
  useEffect(() => {
    ...
    if (viewType === "table") {
      fetchDataByTable(query);
    }
  }, [flagToRefetch, viewType, query]);
  ...
  const fetchDataByTable = async (query) => {
    setDataSet((prev) => ({ ...prev, isFetching: true }));
    try {
      const response = await service.getProspects(query);
